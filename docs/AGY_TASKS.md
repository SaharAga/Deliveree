...
* **Summary**: Revoke exposed Telegram token, purge Git history with git-filter-repo, configure Gitleaks pre-commit hook.

### `TASK-02-CI`: Fix Failing Vitest & Fast-Check Test Suite
* **Priority**: `P0`
* **Status**: `Todo`
* **Squad**: Squad B (`property_test_eng`, `qa_build_verifier`)
* **Summary**: Diagnose and fix the failing annotations in Vitest run. Verify all Zod schemas and fast-check property tests across 2,700+ iterations.
